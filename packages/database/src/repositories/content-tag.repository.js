...
export class ContentTagRepository extends BaseRepository {
     ...
     * 为内容添加标签
     */
    async addTagToContent(contentId, tagId, relevance = 1.0) {
        try {
     ...
     * 获取标签的所有内容
     */
    async getTagContents(tagId, options = {}) {
     ...
     * 批量添加标签到内容
     */
    async batchAddTagsToContent(contentId, tagIds) {
     ...
     * 批量移除内容的标签
     */
    async batchRemoveTagsFromContent(contentId, tagIds) {
     ...
     * 更新内容标签的相关性
     */
    async updateRelevance(contentId, tagId, relevance) {
        try {
     ...
     * 获取内容标签统计
     */
    async getContentTagStats(contentId) {
     ...
     * 清理孤立的内容标签关联
     */
    async cleanupOrphanedAssociations() {
        // 使用原始SQL查询来删除孤立的关联
     ...
     * 获取热门标签组合
     */
    async getPopularTagCombinations(limit = 10) {
        // 这是一个复杂的查询，需要使用原始SQL
