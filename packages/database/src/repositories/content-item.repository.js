 ...
 */
import { ContentStatus, ContentType } from '../generated';
...
export class ContentItemRepository extends BaseRepository {
    ...
    async findMany(filters = {}, options = {}) {
        const { page = 1, limit = 20, orderBy = [{ publishedAt: 'desc' }, { createdAt: 'desc' }], includeRelations = true } = options;
    ...
    async updateStatus(ids, status, userId) {
        ...
        // 为每个内容创建审计日志
        const auditPromises = ids.map(id => this.createAuditLog(id, userId, 'UPDATE_STATUS', 'content', id, { status: 'previous' }, { status }));
     ...
     * 创建审计日志
     */
    async createAuditLog(contentId, userId, action, tableName, recordId, oldValues, newValues) {
     ...
     */
    async getStatistics() {
        const [totalContent, contentByStatus, contentByType, recentContent, duplicateContent,] = await Promise.all([
