...
async function testPolygonIntegration() {
  console.log('🧪 开始测试 Polygon API 集成...\n');

  try {
    // 1. 检查环境变量
    console.log('1. 检查环境变量:');
    const apiKey = process.env.POLYGON_IO_API_KEY;
    console.log(`   POLYGON.IO_API_KEY: ${apiKey ? '已设置' : '未设置'}`);

    if (!apiKey) {
      console.log('❌ 请设置 POLYGON.IO_API_KEY 环境变量');
      return;
    }
    ...
    // 4. 手动触发 Polygon 数据获取任务
    console.log('\n4. 手动触发 Polygon 数据获取任务...');
    const fetchResult = await schedulerService.triggerPolygonFetch();
    console.log('✅ Polygon 数据获取任务完成:', fetchResult);
