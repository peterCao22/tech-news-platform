...
export class ContentFilterController {
     ...
     * 获取当前过滤配置
     */
    async getFilterConfig(req, res, next) {
        try {
     ...
     * 更新过滤配置
     */
    async updateFilterConfig(req, res, next) {
        try {
            const { includeRules, excludeRules, minIncludeScore, maxExcludeScore } = req.body;
            const updateData = {};
            if (includeRules)
                updateData.includeRules = includeRules;
            if (excludeRules)
                updateData.excludeRules = excludeRules;
            if (minIncludeScore !== undefined)
                updateData.minIncludeScore = minIncludeScore;
            if (maxExcludeScore !== undefined)
                updateData.maxExcludeScore = maxExcludeScore;
            contentFilterService.updateConfig(updateData);
            res.json({
                success: true,
                message: '过滤配置更新成功',
                data: contentFilterService.config
            });
        }
        catch (error) {
            next(error);
     ...
     * 测试内容过滤
     */
    async testContentFilter(req, res, next) {
        try {
     ...
     * 批量测试内容过滤
     */
    async batchTestContentFilter(req, res, next) {
        try {
     ...
     * 获取预设的过滤规则模板
     */
    async getFilterTemplates(req, res, next) {
        try {
            const templates = {
                techNews: {
                    name: '科技新闻',
