import { sourceRepository, contentRepository, SourceType } from '@tech-news-platform/database';
...
export class SourceController {
     ...
     * 手动触发RSS源抓取
     */
    fetchSource = async (req, res, next) => {
        try {
            const { id } = req.params;
            // 检查源是否存在且为RSS类型
            const source = await sourceRepository.findById(id);
     ...
     * 批量抓取所有活跃RSS源
     */
    fetchAllSources = async (req, res, next) => {
        try {
     ...
     * 获取信息源统计信息
     */
    getSourceStats = async (req, res, next) => {
        try {
     ...
     * 验证RSS URL
     */
    validateRSSUrl = async (req, res, next) => {
        try {
            const { url } = req.body;
            if (!url) {
                return res.status(400).json({
     ...
     * 获取源的内容列表
     */
    getSourceContent = async (req, res, next) => {
        try {
            const { id } = req.params;
