...
// 测试过滤功能（所有认证用户可用）
router.post('/test', testFilterValidation, validationMiddleware, contentFilterController.testContentFilter);
router.post('/batch-test', batchTestValidation, validationMiddleware, contentFilterController.batchTestContentFilter);
// 需要管理员权限的路由
router.use(authMiddleware.authorize(['admin']));
router.get('/config', contentFilterController.getFilterConfig);
router.put('/config', updateConfigValidation, validationMiddleware, contentFilterController.updateFilterConfig);
