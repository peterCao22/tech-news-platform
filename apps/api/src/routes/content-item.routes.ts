...
router.get('/tags/suggestions',
  query('query').notEmpty().withMessage('查询参数不能为空'),
  validateRequest,
  contentController.getTagSuggestions
);
...
router.post('/:id/tags',
  ...
  addTagsValidation,
  validateRequest,
  contentController.addContentTags
);
...
router.delete('/:id/tags',
  authenticate,
  authorize(['EDITOR', 'ADMIN']),
  removeTagsValidation,
  validateRequest,
  contentController.removeContentTags
);
...
router.post('/check-duplication',
  ...
  validateRequest,
  contentController.checkDuplication
);
