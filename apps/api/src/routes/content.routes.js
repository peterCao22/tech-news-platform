...
// 公开路由（只读）
router.get('/', contentQueryValidation, validationMiddleware, contentController.getContents);
router.get('/stats', contentController.getContentStats);
router.get('/recent', recentContentValidation, validationMiddleware, contentController.getRecentContent);
router.get('/search', searchValidation, validationMiddleware, contentController.searchContent);
router.get('/:id', contentIdValidation, validationMiddleware, contentController.getContent);
// 需要认证的路由
router.use(authMiddleware.authenticate);
// 需要编辑权限的路由
router.use(authMiddleware.authorize(['editor', 'admin']));
router.put('/:id', updateContentValidation, validationMiddleware, contentController.updateContent);
router.patch('/batch/status', batchUpdateValidation, validationMiddleware, contentController.batchUpdateStatus);
// 需要管理员权限的路由
router.use(authMiddleware.authorize(['admin']));
router.delete('/:id', contentIdValidation, validationMiddleware, contentController.deleteContent);
