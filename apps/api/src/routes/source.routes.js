...
router.get('/:id/content', [...sourceIdValidation, ...contentQueryValidation], validationMiddleware, sourceController.getSourceContent);
// 需要认证的路由
router.use(authMiddleware.authenticate);
// RSS URL验证（所有认证用户可用）
router.post('/validate-url', validateUrlValidation, validationMiddleware, sourceController.validateRSSUrl);
// 需要编辑权限的路由 (暂时允许所有认证用户用于测试)
// router.use(authMiddleware.authorize(['editor', 'admin']));
router.post('/', createSourceValidation, validationMiddleware, sourceController.createSource);
router.put('/:id', updateSourceValidation, validationMiddleware, sourceController.updateSource);
// 手动触发抓取
router.post('/:id/fetch', sourceIdValidation, validationMiddleware, sourceController.fetchSource);
// 批量抓取所有活跃源
router.post('/fetch-all', validationMiddleware, sourceController.fetchAllSources);
// 需要管理员权限的路由
router.use(authMiddleware.authorize(['admin']));
router.delete('/:id', sourceIdValidation, validationMiddleware, sourceController.deleteSource);
