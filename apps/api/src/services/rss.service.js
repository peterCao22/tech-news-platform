...
export class RSSService {
     ...
     * 将RSS项目转换为内容数据
     */
    convertRSSItemToContent(item, sourceId) {
        // 提取图片URL
        let imageUrl;
     ...
     * 获取并处理单个RSS源的内容
     */
    async fetchAndProcessSource(sourceId) {
        try {
            // 获取源信息
            ...
            // 第二步：内容质量过滤
            const contentFilterResults = contentFilterService.filterContentBatch(deduplicatedItems.map(item => ({
                title: item.title || '',
            ...
            if (newItems.length > 0) {
                const contentDataList = newItems.map(item => this.convertRSSItemToContent(item, sourceId));
                try {
    ...
    async fetchAllActiveSources() {
        ...
        for (const chunk of chunks) {
            const promises = chunk.map(source => this.fetchAndProcessSource(source.id));
