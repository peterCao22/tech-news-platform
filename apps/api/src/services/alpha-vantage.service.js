...
export class AlphaVantageService {
     ...
     */
    async fetchAndSaveTechNews(limit = 50) {
        const result = {
            success: false,
            totalFetched: 0,
            totalSaved: 0,
     ...
     */
    async fetchAndSaveAINews(limit = 30) {
        const result = {
            success: false,
            totalFetched: 0,
            totalSaved: 0,
     ...
     * 获取并保存特定公司的新闻
     */
    async fetchAndSaveCompanyNews(ticker, limit = 20) {
        const result = {
            success: false,
            totalFetched: 0,
            totalSaved: 0,
     ...
     */
    async getOrCreateAlphaVantageSource() {
        try {
            // 尝试查找现有的 Alpha Vantage 源
            const allSources = await this.sourceRepository.findMany();
            const existingSources = allSources.filter(source => source.name.includes('Alpha Vantage'));
     ...
     */
    async executeFullFetchTask() {
        logger.info('开始执行完整的 Alpha Vantage 数据获取任务');
        const results = {
            techNews: { success: false, totalFetched: 0, totalSaved: 0, errors: [] },
            aiNews: { success: false, totalFetched: 0, totalSaved: 0, errors: [] },
            companyNews: { success: false, totalFetched: 0, totalSaved: 0, errors: [] }
        };
        try {
            // 1. 获取科技新闻
