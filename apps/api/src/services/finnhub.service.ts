// 科技新闻聚合平台 - Finnhub 新闻服务
// 集成 Finnhub API 获取金融新闻和市场数据

import { FinnhubClient, StandardizedNewsItem } from './api/finnhub-client';
...
export class FinnhubNewsService {
   ...
   * 获取 Finnhub 客户端
   */
  private async getClient(): Promise<FinnhubClient> {
    if (!this.finnhubClient) {
      this.finnhubClient = await ApiConfigurationService.getFinnhubClient();
    }
    ...
    try {
      ...
      const sourceData = {
        ...
        config: {
          provider: 'finnhub',
          apiType: 'NEWS',
          description: 'Finnhub 金融新闻 API，提供股票相关新闻和市场数据'
        }
      };

      const newSource = await this.sourceRepository.create(sourceData);
      logger.info('创建新的 Finnhub 源', { id: newSource.id, name: newSource.name });
      
      return newSource;
    } catch (error) {
      logger.error('获取或创建 Finnhub 源失败', { error });
      throw error;
    }
  }
  ...
  public async fetchAndSaveTechNews(limit = 50): Promise<{
    ...
    try {
      logger.info('开始获取 Finnhub 科技新闻', { limit });
      ...
      // 3. 获取或创建 Finnhub 源
      const source = await this.getOrCreateFinnhubSource();
      ...
      const errorMsg = `获取 Finnhub 科技新闻失败: ${error instanceof Error ? error.message : String(error)}`;
      result.errors.push(errorMsg);
      logger.error(errorMsg);
    }
  ...
  public async fetchAndSaveAINews(limit = 30): Promise<{
    ...
    try {
      logger.info('开始获取 Finnhub AI 新闻', { limit });
      ...
      const errorMsg = `获取 Finnhub AI 新闻失败: ${error instanceof Error ? error.message : String(error)}`;
      result.errors.push(errorMsg);
      logger.error(errorMsg);
    }
  ...
  public async fetchAndSaveCompanyNews(symbol: string, limit = 20): Promise<{
    ...
    try {
      logger.info('开始获取 Finnhub 公司新闻', { symbol, limit });
      ...
      for (const newsItem of limitedNews) {
        try {
          ...
          const errorMsg = `保存公司新闻失败: ${newsItem.headline} - ${error instanceof Error ? error.message : String(error)}`;
          result.errors.push(errorMsg);
          logger.error(errorMsg);
        }
      }
      ...
      const errorMsg = `获取 Finnhub 公司新闻失败: ${error instanceof Error ? error.message : String(error)}`;
      result.errors.push(errorMsg);
      logger.error(errorMsg);
    }
  ...
  public async executeFullFetchTask(): Promise<{
    ...
    logger.info('开始执行完整的 Finnhub 数据获取任务');
    ...
    try {
      ...
      logger.error('执行完整的 Finnhub 数据获取任务失败', { error });
      return { success: false, totalSaved, totalErrors, results };
    }
  }
}

export const finnhubService = new FinnhubNewsService();
