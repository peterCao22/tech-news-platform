...
import { 
  ...
  CreateContentItemData as CreateContentData, 
  UpdateContentItemData as UpdateContentData, 
...
export class ContentItemService {
  private contentRepository: ContentItemRepository;
   ...
   * 获取内容列表
   */
  async getContent(filters: ContentSearchFilters = {}, options: {
    page?: number;
    limit?: number;
    orderBy?: any[];
   ...
   * 根据ID获取内容
   */
  async getContentById(id: string, includeRelations: boolean = true): Promise<ContentWithRelations | null> {
   ...
   * 创建内容
   */
  async createContent(data: CreateContentData, userId?: string) {
    // 验证来源是否存在
   ...
   * 更新内容
   */
  async updateContent(id: string, data: UpdateContentData, userId?: string) {
   ...
   * 批量更新内容状态
   */
  async updateContentStatus(ids: string[], status: ContentStatus, userId?: string): Promise<void> {
   ...
   * 为内容添加标签
   */
  async addContentTags(contentId: string, tagIds?: string[], tagNames?: string[]) {
    ...
    const addPromises = tagsToAdd.map(tagId =>
      this.contentTagRepository.addTagToContent(contentId, tagId)
    );

    await Promise.all(addPromises);

    // 更新标签使用次数
    const updateUsagePromises = tagsToAdd.map(tagId =>
      this.tagRepository.incrementUsage(tagId)
    );
   ...
   * 移除内容标签
   */
  async removeContentTags(contentId: string, tagIds: string[]): Promise<void> {
    const content = await this.contentRepository.findById(contentId, false);
    if (!content) {
      throw new Error('内容不存在');
    }
    ...
    // 移除标签关联
    const removePromises = tagIds.map(tagId =>
      this.contentTagRepository.removeTagFromContent(contentId, tagId)
    );

    await Promise.all(removePromises);

    // 更新标签使用次数
    const updateUsagePromises = tagIds.map(tagId =>
      this.tagRepository.decrementUsage(tagId)
    );
   ...
   * 搜索内容
   */
  async searchContent(query: string, filters: Partial<ContentSearchFilters> = {}, options: {
    page?: number;
    limit?: number;
  } = {}) {
    const searchFilters: ContentSearchFilters = {
      ...filters,
      search: query,
    };
   ...
   * 获取相关内容
   */
  async getRelatedContent(contentId: string, limit: number = 5): Promise<ContentWithRelations[]> {
    ...
    if (content.contentTags && content.contentTags.length > 0) {
      filters.tags = content.contentTags.map((ct: any) => ct.tag.name);
    }
   ...
   * 获取热门内容
   */
  async getPopularContent(timeRange: 'day' | 'week' | 'month' = 'week', limit: number = 10) {
    const now = new Date();
    let dateFrom: Date;
   ...
   * 获取最新内容
   */
  async getLatestContent(limit: number = 10) {
    const filters: ContentSearchFilters = {
      status: ContentStatus.PUBLISHED,
    };
   ...
   * 按分类获取内容
   */
  async getContentByCategory(category: string, options: {
    page?: number;
    limit?: number;
  } = {}) {
    const filters: ContentSearchFilters = {
   ...
   * 按标签获取内容
   */
  async getContentByTags(tags: string[], options: {
    page?: number;
    limit?: number;
  } = {}) {
    const filters: ContentSearchFilters = {
   ...
   * 批量处理内容
   */
  async batchProcessContent(action: 'approve' | 'reject' | 'archive', contentIds: string[], userId?: string) {
