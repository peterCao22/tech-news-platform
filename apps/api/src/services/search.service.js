...
export class SearchService {
    ...
    async searchContent(query, filters = {}, options = {}) {
        ...
        const { page = 1, limit = 20, sortBy = 'relevance', sortOrder = 'desc', includeHighlights = true, } = options;
        ...
        // 计算相关性评分和高亮
        const processedResults = await this.processSearchResults(results, query, includeHighlights);
     ...
     * 执行搜索查询
     */
    async executeSearch(conditions, skip, limit, sortBy, sortOrder) {
        // 构建排序条件
        let orderBy = [];
        switch (sortBy) {
            case 'date':
     ...
     * 计算搜索结果数量
     */
    async countSearchResults(conditions) {
     ...
     * 处理搜索结果（计算相关性和高亮）
     */
    async processSearchResults(results, query, includeHighlights) {
        return results.map(result => {
            ...
            return {
                ...
                tags: result.contentTags.map((ct) => ct.tag.name),
                score: result.score,
    ...
    calculateRelevanceScore(content, query) {
        ...
        // 标签匹配
        const tagNames = content.contentTags.map((ct) => ct.tag.name.toLowerCase());
        const tagMatches = queryTerms.filter(term => tagNames.some((tagName) => tagName.includes(term))).length;
        score += tagMatches * 2;
        // 关键词匹配
        const keywordMatches = content.keywords?.filter((keyword) => queryTerms.some(term => keyword.toLowerCase().includes(term))).length || 0;
    ...
    generateHighlights(content, query) {
        ...
        if (content.description) {
            highlights.description = this.highlightText(this.truncateText(content.description, 200), queryTerms);
        }
        // 高亮内容片段
        if (content.content) {
     ...
     * 更新搜索索引
     */
    async updateSearchIndex(contentId) {
     ...
     * 生成搜索向量
     */
    generateSearchVector(title, description, content) {
     ...
     * 批量更新搜索索引
     */
    async batchUpdateSearchIndex(contentIds) {
