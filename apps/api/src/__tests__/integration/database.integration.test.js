...
describe('Database Integration Tests', () => {
    ...
    describe('Content Management', () => {
        ...
        it('应该更新内容并创建新版本', async () => {
            ...
            const updatedContent = await contentRepository.update('test-content-update', updateData, 'test-user-2');
    ...
    describe('Content-Tag Relationships', () => {
        ...
        it('应该防止重复关联', async () => {
            ...
            // 尝试重复添加相同标签
            await expect(contentTagRepository.addTagToContent(testContent.id, testTags[0].id)).rejects.toThrow();
        });
        it('应该正确查找共同标签', async () => {
            // 创建另一个内容
    ...
    describe('Complex Queries and Transactions', () => {
        it('应该正确执行复杂的搜索查询', async () => {
            ...
            // 测试分类过滤
            const categoryResult = await contentRepository.findMany({ category: 'AI' }, { page: 1, limit: 10 });
