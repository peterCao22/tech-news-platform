...
const mockContentService = ContentItemService;
const mockTagService = TagService;
describe('ContentItemController Integration Tests', () => {
    let app;
    let contentServiceInstance;
    let tagServiceInstance;
    beforeAll(() => {
        app = express();
    ...
    describe('GET /api/content-items', () => {
        ...
        it('应该返回内容列表', async () => {
            contentServiceInstance.getContent.mockResolvedValue(mockContentList);
            ...
            expect(contentServiceInstance.getContent).toHaveBeenCalledWith(expect.objectContaining({}), expect.objectContaining({
                page: 1,
                limit: 20,
            }));
        });
        it('应该应用查询参数过滤', async () => {
            ...
            await request(app)
                ...
                .expect(200);
            expect(contentServiceInstance.getContent).toHaveBeenCalledWith(expect.objectContaining({
                status: ContentStatus.PUBLISHED,
                type: ContentType.NEWS,
                category: 'AI',
                search: '测试',
    ...
    describe('GET /api/content-items/:id', () => {
        ...
        it('应该返回内容详情', async () => {
            contentServiceInstance.getContentById.mockResolvedValue(mockContent);
    ...
    describe('POST /api/content-items', () => {
        ...
        it('应该成功创建内容', async () => {
            contentServiceInstance.createContent.mockResolvedValue(mockCreatedContent);
            const response = await request(app)
            ...
            expect(contentServiceInstance.createContent).toHaveBeenCalledWith(mockContentData, 'user-123');
        });
        ...
        it('当检测到重复内容时应该返回409', async () => {
            contentServiceInstance.createContent.mockRejectedValue(new Error('检测到重复内容，相似度: 95.0%'));
            const response = await request(app)
                .post('/api/content-items')
                .send(mockContentData)
                .expect(409);
    ...
    describe('PUT /api/content-items/:id', () => {
        ...
        it('应该成功更新内容', async () => {
            contentServiceInstance.updateContent.mockResolvedValue(mockUpdatedContent);
            const response = await request(app)
            ...
            expect(contentServiceInstance.updateContent).toHaveBeenCalledWith('content-123', mockUpdateData, 'user-123');
        });
        it('当内容不存在时应该返回404', async () => {
    ...
    describe('DELETE /api/content-items/:id', () => {
        it('应该成功删除内容', async () => {
            ...
            expect(contentServiceInstance.deleteContent).toHaveBeenCalledWith('content-123', 'user-123');
        });
    });
    describe('PATCH /api/content-items/batch/status', () => {
        it('应该批量更新内容状态', async () => {
            ...
            expect(contentServiceInstance.updateContentStatus).toHaveBeenCalledWith(requestData.ids, requestData.status, 'user-123');
        });
    ...
    describe('POST /api/content-items/:id/tags', () => {
        ...
        it('应该为内容添加标签', async () => {
            contentServiceInstance.addContentTags.mockResolvedValue(mockUpdatedContent);
            const response = await request(app)
            ...
            expect(contentServiceInstance.addContentTags).toHaveBeenCalledWith('content-123', ['tag-1', 'tag-2'], undefined);
        });
        it('应该支持通过标签名称添加', async () => {
            ...
            await request(app)
                ...
                .expect(200);
            expect(contentServiceInstance.addContentTags).toHaveBeenCalledWith('content-123', undefined, ['新标签1', '新标签2']);
        });
    ...
    describe('DELETE /api/content-items/:id/tags', () => {
        it('应该移除内容标签', async () => {
            ...
            expect(contentServiceInstance.removeContentTags).toHaveBeenCalledWith('content-123', ['tag-1', 'tag-2']);
        });
    });
    describe('POST /api/content-items/check-duplication', () => {
        ...
        it('应该检查内容重复', async () => {
            ...
            expect(contentServiceInstance.checkDuplication).toHaveBeenCalledWith('测试标题', '测试内容', 'https://example.com');
        });
    ...
    describe('GET /api/content-items/statistics', () => {
        ...
        it('应该返回内容统计信息', async () => {
            contentServiceInstance.getStatistics.mockResolvedValue(mockStats);
            const response = await request(app)
    ...
    describe('Tag Management', () => {
        describe('GET /api/content-items/tags', () => {
            ...
            it('应该返回标签列表', async () => {
                tagServiceInstance.getTags.mockResolvedValue(mockTagList);
                const response = await request(app)
                ...
                expect(tagServiceInstance.getTags).toHaveBeenCalledWith(expect.objectContaining({}), expect.objectContaining({
                    page: 1,
                    limit: 50,
                }));
            });
        ...
        describe('POST /api/content-items/tags', () => {
            ...
            it('应该成功创建标签', async () => {
                tagServiceInstance.createTag.mockResolvedValue(mockCreatedTag);
                const response = await request(app)
        ...
        describe('GET /api/content-items/tags/suggestions', () => {
            ...
            it('应该返回标签建议', async () => {
                tagServiceInstance.searchSuggestions.mockResolvedValue(mockSuggestions);
                const response = await request(app)
                ...
                expect(tagServiceInstance.searchSuggestions).toHaveBeenCalledWith('AI', 5, undefined);
            });
        ...
        describe('GET /api/content-items/tags/popular', () => {
            ...
            it('应该返回热门标签', async () => {
                tagServiceInstance.getPopularTags.mockResolvedValue(mockPopularTags);
                const response = await request(app)
