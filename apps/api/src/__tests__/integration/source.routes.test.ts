...
describe('RSS源管理API集成测试', () => {
  ...
  describe('GET /api/sources', () => {
    it('应该返回所有RSS源列表（公开访问）', async () => {
      ...
      (sourceRepository.findMany as any).mockResolvedValue(mockSources);
    ...
    it('应该支持分页参数', async () => {
      (sourceRepository.findMany as any).mockResolvedValue([]);

      await request(app)
        .get('/api/sources?page=2&limit=10')
        .expect(200);

      expect(sourceRepository.findMany).toHaveBeenCalledWith(
  ...
  describe('GET /api/sources/:id', () => {
    it('应该返回指定的RSS源详情', async () => {
      ...
      (sourceRepository.findById as any).mockResolvedValue(mockSource);
    ...
    it('应该处理源不存在的情况', async () => {
      (sourceRepository.findById as any).mockResolvedValue(null);
  ...
  describe('POST /api/sources', () => {
    ...
    it('应该成功创建新的RSS源（需要认证）', async () => {
      ...
      (sourceRepository.create as any).mockResolvedValue(mockCreatedSource);
      ...
      expect(sourceRepository.create).toHaveBeenCalledWith(
        expect.objectContaining(validSourceData)
      );
    });
  ...
  describe('PUT /api/sources/:id', () => {
    ...
    it('应该成功更新RSS源', async () => {
      ...
      (sourceRepository.update as any).mockResolvedValue(mockUpdatedSource);
    ...
    it('应该处理源不存在的情况', async () => {
      (sourceRepository.update as any).mockResolvedValue(null);
  ...
  describe('DELETE /api/sources/:id', () => {
    it('应该允许管理员删除RSS源', async () => {
      (sourceRepository.delete as any).mockResolvedValue(true);
    ...
    it('应该处理源不存在的情况', async () => {
      (sourceRepository.delete as any).mockResolvedValue(false);
  ...
  describe('POST /api/sources/validate-url', () => {
    it('应该验证有效的RSS URL', async () => {
      (rssService.validateRSSUrl as any).mockResolvedValue({
        valid: true,
        feedInfo: {
          title: 'Valid Feed',
          itemCount: 10,
  ...
  describe('POST /api/sources/:id/fetch', () => {
    it('应该手动触发RSS源抓取', async () => {
      (rssService.fetchAndProcessSource as any).mockResolvedValue({
        success: true,
        newItemsCount: 5,
      });
  ...
  describe('POST /api/sources/fetch-all', () => {
    it('应该批量抓取所有活跃源', async () => {
      (rssService.fetchAllActiveSources as any).mockResolvedValue({
        totalSources: 3,
        successfulSources: 2,
        totalNewItems: 15,
        results: [
  ...
  describe('GET /api/sources/stats', () => {
    it('应该返回RSS源统计信息', async () => {
      ...
      (sourceRepository.getStats as any).mockResolvedValue(mockStats);
  ...
  describe('错误处理', () => {
    it('应该处理数据库错误', async () => {
      (sourceRepository.findMany as any).mockRejectedValue(new Error('Database error'));
  ...
  describe('输入验证', () => {
    it('应该清理恶意输入', async () => {
      ...
      (sourceRepository.create as any).mockResolvedValue({
        id: 'source-new',
        ...maliciousData,
        status: 'ACTIVE',
      });
