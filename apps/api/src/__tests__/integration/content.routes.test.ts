...
describe('内容管理API集成测试', () => {
  ...
  beforeEach(() => {
    ...
    // 模拟用户查找
    (UserRepository.findById as any).mockImplementation((id: string) => {
      if (id === 'user-123') {
  ...
  describe('GET /api/content', () => {
    it('应该返回内容列表（公开访问）', async () => {
      ...
      (contentRepository.findMany as any).mockResolvedValue({
        data: mockContents,
    ...
    it('应该支持分页参数', async () => {
      ...
      expect(contentRepository.findMany).toHaveBeenCalledWith(
  ...
  describe('GET /api/content/:id', () => {
    it('应该返回指定内容的详情', async () => {
      ...
      (contentRepository.findById as any).mockResolvedValue(mockContent);
    ...
    it('应该处理内容不存在的情况', async () => {
      (contentRepository.findById as any).mockResolvedValue(null);
  ...
  describe('GET /api/content/recent', () => {
    it('应该返回最近的内容', async () => {
      ...
      (contentRepository.findRecent as any).mockResolvedValue(mockRecentContent);
    ...
    it('应该支持自定义时间范围', async () => {
      (contentRepository.findRecent as any).mockResolvedValue([]);
  ...
  describe('GET /api/content/search', () => {
    it('应该支持内容搜索', async () => {
      ...
      (contentRepository.findMany as any).mockResolvedValue(mockSearchResults);
  ...
  describe('PUT /api/content/:id', () => {
    ...
    it('应该允许认证用户更新内容', async () => {
      ...
      (contentRepository.update as any).mockResolvedValue(mockUpdatedContent);
    ...
    it('应该处理内容不存在的情况', async () => {
      (contentRepository.update as any).mockResolvedValue(null);
  ...
  describe('DELETE /api/content/:id', () => {
    it('应该允许管理员删除内容', async () => {
      (contentRepository.delete as any).mockResolvedValue(true);
  ...
  describe('POST /api/content/batch-update-status', () => {
    it('应该允许批量更新内容状态', async () => {
      ...
      (contentRepository.updateStatus as any).mockResolvedValue({
        count: 3,
      });
      ...
      expect(contentRepository.updateStatus).toHaveBeenCalledWith(
        expect.arrayContaining(['content-1', 'content-2', 'content-3']),
        expect.any(String)
      );
    });
  ...
  describe('GET /api/content/stats', () => {
    it('应该返回内容统计信息', async () => {
      ...
      (contentRepository.getStats as any).mockResolvedValue(mockStats);
  ...
  describe('错误处理', () => {
    it('应该处理数据库错误', async () => {
      (contentRepository.findMany as any).mockRejectedValue(new Error('Database error'));
    ...
    it('应该处理搜索服务错误', async () => {
      (contentRepository.findMany as any).mockRejectedValue(new Error('Search service error'));
