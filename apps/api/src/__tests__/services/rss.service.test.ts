...
describe('RSSService', () => {
  ...
  describe('parseFeed', () => {
    ...
    it('应该处理RSS解析错误', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        '解析RSS源失败: https://invalid-url.com/rss',
        { error: 'Network error' }
      );
    });
  ...
  describe('fetchAndProcessSource', () => {
    ...
    beforeEach(() => {
      ...
      (contentFilterService.filterContentBatch as any).mockReturnValue([
      ...
      (contentFilterService.getFilterStats as any).mockReturnValue({
        total: 2,
        filtered: 0,
        kept: 2,
        filterRate: 0,
      ...
      (contentRepository.createMany as any).mockResolvedValue({ count: 2 });
      (sourceRepository.updateFetchStats as any).mockResolvedValue(undefined);
    });
    ...
    it('应该处理源不存在的情况', async () => {
      ...
      expect(sourceRepository.updateFetchStats).toHaveBeenCalledWith(
        'invalid-source',
        false,
        'Source not found or URL missing'
      );
    });
    ...
    it('应该正确处理去重逻辑', async () => {
      ...
      (contentRepository.findRecent as any).mockResolvedValue(existingContent);
    ...
    it('应该处理内容过滤', async () => {
      ...
      (contentRepository.createMany as any).mockResolvedValue({ count: 1 });

      const result = await rssService.fetchAndProcessSource('source-123');

      expect(result.newItemsCount).toBe(1);
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('❌ 过滤: Article 1')
      );
      expect(logger.info).toHaveBeenCalledWith(
        expect.stringContaining('✅ 保留: Article 2')
      );
    });

    it('应该处理批量保存失败并尝试逐个保存', async () => {
      (contentRepository.createMany as any).mockRejectedValue(new Error('Batch save failed'));
      (contentRepository.create as any)
  ...
  describe('fetchAllActiveSources', () => {
    it('应该并发处理多个活跃源', async () => {
      ...
      (sourceRepository.findActiveRssSources as any).mockResolvedValue(mockSources);
