...
describe('ContentFilterService', () => {
    ...
    describe('shouldFilterContent', () => {
        it('应该保留包含科技新闻关键词的内容', () => {
            const result = contentFilterService.shouldFilterContent('腾讯发布并开源新一代生图模型"混元图像3.0"', '腾讯公司今日宣布发布新一代人工智能生图模型', '');
        ...
        it('应该保留包含加密货币关键词的内容', () => {
            const result = contentFilterService.shouldFilterContent('加密货币价格走低，以太坊与比特币现货ETF创单周资金流出纪录', '加密货币市场出现波动，比特币和以太坊价格下跌', '');
            expect(result.shouldFilter).toBe(false);
            expect(result.includeScore).toBeGreaterThanOrEqual(0.1);
            expect(result.reason).toContain('符合科技新闻标准');
        });
        it('应该保留包含融资投资关键词的内容', () => {
            const result = contentFilterService.shouldFilterContent('"乐享科技"完成2亿元"天使++"轮融资', '科技公司获得新一轮投资，用于产品研发', '');
            expect(result.shouldFilter).toBe(false);
            expect(result.includeScore).toBeGreaterThanOrEqual(0.1);
            expect(result.reason).toContain('符合科技新闻标准');
        });
        it('应该过滤包含编程开发关键词的内容', () => {
            const result = contentFilterService.shouldFilterContent('React 18新特性详解：并发渲染和自动批处理', '本文介绍React 18的新功能，包括并发渲染、自动批处理等开发者特性', 'function Component() { return <div>Hello</div>; }');
        ...
        it('应该过滤包含算法和数据结构的内容', () => {
            const result = contentFilterService.shouldFilterContent('深度学习算法优化：从理论到实践', '本文讨论机器学习算法的优化方法和数据结构设计', '');
            expect(result.shouldFilter).toBe(true);
        ...
        it('应该处理只有标题的内容', () => {
            const result = contentFilterService.shouldFilterContent('人工智能技术突破', '', '');
            expect(result.shouldFilter).toBe(false);
            expect(result.includeScore).toBeGreaterThan(0);
        });
        it('应该正确计算混合内容的分数', () => {
            // 包含既有包含关键词又有排除关键词的内容
            const result = contentFilterService.shouldFilterContent('人工智能开发框架TensorFlow 2.0发布', '谷歌发布了新版本的机器学习框架，包含多项开发者功能改进', '');
            // 应该根据包含和排除分数的平衡来决定
            expect(result.includeScore).toBeGreaterThan(0);
    ...
    describe('边界情况测试', () => {
        it('应该处理极长的文本', () => {
            ...
            const result = contentFilterService.shouldFilterContent(longText, '', '');
            expect(result.includeScore).toBeGreaterThan(0);
        });
        it('应该处理特殊字符', () => {
            const result = contentFilterService.shouldFilterContent('人工智能@#$%^&*()技术突破！！！', '科技新闻：AI发展....', '');
            expect(result.shouldFilter).toBe(false);
            expect(result.includeScore).toBeGreaterThan(0);
        });
        it('应该处理Unicode字符', () => {
            const result = contentFilterService.shouldFilterContent('🤖 人工智能技术 🚀 突破性进展', 'AI技术在各领域应用 💡', '');
            expect(result.shouldFilter).toBe(false);
            expect(result.includeScore).toBeGreaterThan(0);
        });
    });
    describe('配置测试', () => {
        it('应该使用正确的阈值配置', () => {
            const config = contentFilterService.config;
