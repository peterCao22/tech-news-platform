...
export function RoleGuard({ children, role, permission, fallback = null, requireAll = false, }) {
...
export function AdminOnly({ children, fallback = null }) {
    return (<RoleGuard role="ADMIN" fallback={fallback}>
      {children}
    </RoleGuard>);
}
// 编辑者及以上权限组件
export function EditorOnly({ children, fallback = null }) {
    return (<RoleGuard role="EDITOR" fallback={fallback}>
      {children}
    </RoleGuard>);
}
// 已认证用户组件
