...
import { ArrowLeft, Check, AlertCircle, Loader2, Save } from 'lucide-react';
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <Card>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {source.type === 'RSS' && (<div>
                    ...
                    <div className="flex gap-2">
                      <Input type="url" value={formData.url || ''} onChange={(e) => handleInputChange('url', e.target.value)} placeholder="https://example.com/rss.xml" className="flex-1" required/>
                      <Button type="button" variant="outline" onClick={handleValidateUrl} disabled={validating || !formData.url}>
                        {validating ? (<Loader2 className="h-4 w-4 animate-spin"/>) : ('验证')}
                      </Button>
                    </div>
                    
                    {/* 验证结果 */}
                    {validation && (<div className={`mt-2 p-3 rounded-md ${validation.valid
                    ? 'bg-green-50 border border-green-200'
                    : 'bg-red-50 border border-red-200'}`}>
                        <div className="flex items-start gap-2">
                          {validation.valid ? (<Check className="h-5 w-5 text-green-600 mt-0.5"/>) : (<AlertCircle className="h-5 w-5 text-red-600 mt-0.5"/>)}
                          <div className="flex-1">
                            {validation.valid ? (<div>
                                <p className="text-green-800 font-medium">RSS验证成功</p>
                                {validation.title && (<p className="text-green-700 text-sm mt-1">
                                    <strong>标题:</strong> {validation.title}
                                  </p>)}
                                {validation.description && (<p className="text-green-700 text-sm mt-1">
                                    <strong>描述:</strong> {validation.description}
                                  </p>)}
                                {validation.itemCount !== undefined && (<p className="text-green-700 text-sm mt-1">
                ...
                <div>
                  ...
                  <Input type="text" value={formData.name || ''} onChange={(e) => handleInputChange('name', e.target.value)} placeholder="例如: 科技新闻RSS" required/>
                </div>
                ...
                <div>
                  ...
                  <select value={formData.status} onChange={(e) => handleInputChange('status', e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                <div>
                  ...
                  <textarea value={formData.config?.description || ''} onChange={(e) => handleInputChange('config', {
            ...formData.config,
            description: e.target.value
        })} placeholder="简要描述这个RSS源的内容..." rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"/>
                </div>
                ...
                <div>
                  ...
                  <select value={formData.config?.category || 'tech'} onChange={(e) => handleInputChange('config', {
            ...formData.config,
            category: e.target.value
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  <Button type="submit" disabled={saving || (source.type === 'RSS' && validation && !validation.valid)} loading={saving}>
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              {source.lastError && (<div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-md">
