...
import { User, Settings, LogOut, Bell, BookOpen, Rss, FileText, TrendingUp, Clock, Star, Filter, Search, Menu, X, Key } from 'lucide-react';
...
export default function DashboardPage() {
    ...
    return (<div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center">
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="md:hidden p-2 rounded-md text-gray-600 hover:text-gray-900 hover:bg-gray-100">
            ...
            <div className="hidden md:block flex-1 max-w-lg mx-8">
              <div className="relative">
                ...
                <Input type="text" placeholder="搜索新闻..." className="pl-10 pr-4"/>
              </div>
            </div>
      ...
      <div className="flex">
          ...
          <div className="flex flex-col h-full">
            <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
              <nav className="mt-5 flex-1 px-2 space-y-1">
                <Link href="/dashboard" className="bg-blue-100 text-blue-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
                  <TrendingUp className="text-blue-500 mr-3 flex-shrink-0 h-5 w-5"/>
                  仪表板
                </Link>
                
                <Link href="/news" className="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
                  <BookOpen className="text-gray-400 group-hover:text-gray-500 mr-3 flex-shrink-0 h-5 w-5"/>
                  新闻列表
                </Link>
                
                <Link href="/favorites" className="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
                  <Star className="text-gray-400 group-hover:text-gray-500 mr-3 flex-shrink-0 h-5 w-5"/>
                  我的收藏
                </Link>
                
                <Link href="/sources" className="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
                  <Rss className="text-gray-400 group-hover:text-gray-500 mr-3 flex-shrink-0 h-5 w-5"/>
                  RSS源管理
                </Link>
                
                <Link href="/content" className="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
                  <FileText className="text-gray-400 group-hover:text-gray-500 mr-3 flex-shrink-0 h-5 w-5"/>
                  内容管理
                </Link>
                
                {user?.role === 'ADMIN' && (<Link href="/api-configs" className="text-gray-600 hover:bg-gray-50 hover:text-gray-900 group flex items-center px-2 py-2 text-sm font-medium rounded-md">
            ...
            <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
              <Button onClick={handleLogout} variant="ghost" className="w-full justify-start text-gray-600 hover:text-gray-900">
        ...
        ].map((item, index) => (<div key={index} className="flex space-x-4 hover:bg-gray-50 p-2 rounded-lg cursor-pointer transition-colors">
      ...
      {/* 移动端侧边栏遮罩 */}
      {isSidebarOpen && (<div className="md:hidden fixed inset-0 z-40 bg-gray-600 bg-opacity-75" onClick={() => setIsSidebarOpen(false)}/>)}
    </div>);
}
