...
import { ArrowLeft, ExternalLink, Tag, User, Clock, CheckCircle, XCircle, AlertCircle, FileText, RefreshCw, Trash2, Share2 } from 'lucide-react';
...
export default function ContentDetailPage() {
    ...
    const renderContent = (htmlContent) => {
        ...
        return (<div className="prose prose-gray max-w-none" dangerouslySetInnerHTML={{ __html: cleanHtml }}/>);
    };
    useEffect(() => {
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              {/* 图片 */}
              {content.imageUrl && (<div className="relative">
                  <img src={content.imageUrl} alt={content.title} className="w-full max-h-96 object-cover rounded-lg" onError={(e) => {
              ...
              {/* 标签 */}
              {content.tags.length > 0 && (<div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">标签</h3>
                  <div className="flex flex-wrap gap-2">
                    {content.tags.map((tag, index) => (<span key={index} className="px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-full">
                        {tag}
                      </span>))}
                  </div>
                </div>)}
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center gap-4">
                ...
                <select value={content.status} onChange={(e) => handleUpdateStatus(e.target.value)} disabled={updating} className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50">
                ...
                {updating && (<RefreshCw className="h-4 w-4 animate-spin text-blue-500"/>)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                {content.sourceUrl && (<div className="md:col-span-2">
                    <span className="font-medium text-gray-700">原文链接:</span>
                    <a href={content.sourceUrl} target="_blank" rel="noopener noreferrer" className="ml-2 text-blue-600 hover:underline break-all">
                      {content.sourceUrl}
                    </a>
                  </div>)}
              </div>
