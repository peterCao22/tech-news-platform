...
import { apiConfigService, CreateApiConfigData } from '../../../services/api-config.service';

const CreateApiConfigPage: React.FC = () => {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [testing, setTesting] = useState(false);
  const [showSensitiveFields, setShowSensitiveFields] = useState(false);
  
  const [formData, setFormData] = useState<CreateApiConfigData>({
    name: '',
    provider: '',
    baseUrl: '',
    authType: 'API_KEY',
  ...
  // 处理表单字段变化
  const handleFieldChange = (field: keyof CreateApiConfigData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <Input
                    type="number"
                    value={formData.rateLimit?.maxRequests || 100}
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    value={formData.rateLimit?.windowMs || 60000}
                    onChange={(e) => handleFieldChange('rateLimit', {
                      ...formData.rateLimit,
