...
import { ArrowLeft, Save, TestTube, Eye, EyeOff, AlertTriangle, Trash2 } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { apiConfigService } from '../../../../services/api-config.service';
const EditApiConfigPage = () => {
    ...
    return (<div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <Button variant="outline" onClick={handleDeleteConfig} className="flex items-center gap-2 text-red-600 hover:text-red-700">
      ...
      <Card className="p-6 mb-8">
        ...
        {config.lastCallAt && (<div className="mt-4 pt-4 border-t border-gray-200">
            <p className="text-sm text-gray-600">
              最后调用时间: {new Date(config.lastCallAt).toLocaleString()}
            </p>
          </div>)}
        {config.lastError && (<div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <Card className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Select value={formData.status || config.status} onValueChange={(value) => handleFieldChange('status', value)}>
            ...
            <div>
              ...
              <Input type="text" value={formData.provider || config.provider} onChange={(e) => handleFieldChange('provider', e.target.value)} placeholder="API提供商"/>
            </div>
            ...
            <div>
              ...
              <Select value={formData.authType || config.authType} onValueChange={(value) => handleFieldChange('authType', value)}>
            ...
            <div className="md:col-span-2">
              ...
              <Input type="url" value={formData.baseUrl || config.baseUrl} onChange={(e) => handleFieldChange('baseUrl', e.target.value)} placeholder="https://api.example.com"/>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-6">
            {/* API Key 认证 */}
            {(formData.authType || config.authType) === 'API_KEY' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    API密钥
                  </label>
                  <Input type={showSensitiveFields ? "text" : "password"} value={formData.apiKey || ''} onChange={(e) => handleFieldChange('apiKey', e.target.value)} placeholder="输入新的API密钥（留空保持不变）"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    头部名称
                  </label>
                  <Input type="text" value={formData.headerName || config.headerName || ''} onChange={(e) => handleFieldChange('headerName', e.target.value)} placeholder="X-API-Key"/>
                </div>
              </div>)}

            {/* Bearer Token 认证 */}
            {(formData.authType || config.authType) === 'BEARER_TOKEN' && (<div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Bearer Token
                </label>
                <Input type={showSensitiveFields ? "text" : "password"} value={formData.token || ''} onChange={(e) => handleFieldChange('token', e.target.value)} placeholder="输入新的Bearer Token（留空保持不变）"/>
              </div>)}

            {/* Basic Auth 认证 */}
            {(formData.authType || config.authType) === 'BASIC_AUTH' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    用户名
                  </label>
                  <Input type="text" value={formData.username || ''} onChange={(e) => handleFieldChange('username', e.target.value)} placeholder="输入新的用户名（留空保持不变）"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    密码
                  </label>
                  <Input type={showSensitiveFields ? "text" : "password"} value={formData.password || ''} onChange={(e) => handleFieldChange('password', e.target.value)} placeholder="输入新的密码（留空保持不变）"/>
                </div>
              </div>)}
          </div>
        </Card>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <Input type="number" value={formData.rateLimit?.maxRequests || config.rateLimit?.maxRequests || 100} onChange={(e) => handleFieldChange('rateLimit', {
            ...formData.rateLimit,
            maxRequests: parseInt(e.target.value)
                ...
                <div>
                  ...
                  <Input type="number" value={formData.rateLimit?.windowMs || config.rateLimit?.windowMs || 60000} onChange={(e) => handleFieldChange('rateLimit', {
            ...formData.rateLimit,
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <Input type="number" value={formData.timeout || config.timeout || 30000} onChange={(e) => handleFieldChange('timeout', parseInt(e.target.value))} min="1000"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    重试次数
                  </label>
                  <Input type="number" value={formData.retryAttempts || config.retryAttempts || 3} onChange={(e) => handleFieldChange('retryAttempts', parseInt(e.target.value))} min="0" max="10"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    重试延迟 (毫秒)
                  </label>
                  <Input type="number" value={formData.retryDelay || config.retryDelay || 1000} onChange={(e) => handleFieldChange('retryDelay', parseInt(e.target.value))} min="100"/>
                </div>
              </div>
            </div>
