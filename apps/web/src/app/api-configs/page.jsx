...
import { Plus, Settings, Trash2, TestTube, CheckCircle, XCircle, Clock, AlertTriangle, Search } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { apiConfigService } from '../../services/api-config.service';
const ApiConfigsPage = () => {
    ...
    return (<div className="container mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <Button onClick={() => router.push('/api-configs/create')} className="flex items-center gap-2">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <Input type="text" placeholder="搜索配置名称或提供商..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10"/>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <Select value={providerFilter} onValueChange={setProviderFilter}>
              ...
              {uniqueProviders.map(provider => (<option key={provider} value={provider}>{provider}</option>))}
            </Select>
          </div>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredConfigs.map((config) => (<Card key={config.id} className="p-6">
            ...
            <div className="space-y-2 mb-4">
              ...
              {config.lastCallAt && (<div className="flex justify-between text-sm">
            ...
            {config.lastError && (<div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md">
            ...
            <div className="flex justify-between items-center">
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => handleTestConfig(config.id)} className="flex items-center gap-1">
                  <TestTube className="w-3 h-3"/>
                  测试
                </Button>
                <Button variant="outline" size="sm" onClick={() => router.push(`/api-configs/${config.id}/edit`)} className="flex items-center gap-1">
                  <Settings className="w-3 h-3"/>
                  编辑
                </Button>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => handleToggleStatus(config)} className={`${config.status === 'ACTIVE'
                ? 'text-red-600 hover:text-red-700'
                : 'text-green-600 hover:text-green-700'}`}>
                  {config.status === 'ACTIVE' ? '禁用' : '启用'}
                </Button>
                <Button variant="outline" size="sm" onClick={() => handleDeleteConfig(config.id, config.name)} className="text-red-600 hover:text-red-700">
      ...
      {filteredConfigs.length === 0 && (<Card className="p-12 text-center">
          ...
          {(!searchTerm && statusFilter === 'all' && providerFilter === 'all') && (<Button onClick={() => router.push('/api-configs/create')} className="flex items-center gap-2 mx-auto">
              <Plus className="w-4 h-4"/>
              创建API配置
            </Button>)}
        </Card>)}
